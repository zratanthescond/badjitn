...
export default function HelpPage() {
  ...
  const popularArticles = [
    ...
    { id: "organisation-verification", category: "getting-started", readTime: 3 },
    ...
    >
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="text-center mb-12">
          <div
            className={`flex justify-center mb-6 ${isRTL ? "flex-row-reverse" : ""
              }`}
          >
          ...
          <h1
            className={`text-5xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent mb-4 ${isRTL ? "font-arabic" : ""
              }`}
          >
            {t("title")}
          </h1>
          ...
          <Card className="glass bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-white/20 dark:border-slate-700/50 rounded-3xl shadow-xl max-w-2xl mx-auto">
            <CardContent className="p-8">
              <div className="relative">
                <Search
                  className={`absolute top-4 h-5 w-5 text-muted-foreground ${isRTL ? "right-4" : "left-4"
                    }`}
                />
                <Input
                  ...
                  className={`glass bg-white/60 dark:bg-slate-800/60 border-white/30 dark:border-slate-700/50 rounded-2xl h-14 text-lg ${isRTL ? "pr-12 font-arabic text-right" : "pl-12"
                    }`}
                />
              </div>
              <div
              ...
              >
                ...
                {["events", "payments", "account", "organisations"].map((term) => (
                  <Button
                    key={term}
                    variant="outline"
                    size="sm"
                    onClick={() => setSearchQuery(t(`search.terms.${term}`))}
                    className={`glass bg-white/40 dark:bg-slate-800/40 border-white/30 dark:border-slate-700/50 rounded-full text-xs hover:bg-white/60 dark:hover:bg-slate-700/60 ${isRTL ? "font-arabic" : ""
                      }`}
                  >
                    {t(`search.terms.${term}`)}
                  </Button>
        ...
        <div className="mb-12">
          <h2
            className={`text-2xl font-bold mb-6 text-center ${isRTL ? "font-arabic" : ""
              }`}
          >
            {t("quickLinks.title")}
          </h2>
              ...
              return (
                ...
                >
                  <CardContent className="p-6 text-center">
                    ...
                    <h3
                      className={`font-semibold mb-2 ${isRTL ? "font-arabic" : ""
                        }`}
                    >
                    ...
                    <CardContent>
                      <div className="space-y-2">
                        {Array.from({ length: 3 }).map((_, index) => (
                          <div
                            key={index}
                            className={`flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors cursor-pointer ${isRTL ? "flex-row-reverse" : ""
                              }`}
                          >
                            <ChevronRight
                      ...
                      <Button
                        variant="ghost"
                        className={`w-full mt-4 glass bg-white/40 dark:bg-slate-800/40 hover:bg-white/60 dark:hover:bg-slate-700/60 rounded-full ${isRTL ? "font-arabic" : ""
                          }`}
                        asChild
                      >
                        <Link href={`/help/${category.id}`}>
                          ...
                          <ChevronRight
                            className={`h-4 w-4 ml-2 ${isRTL ? "rotate-180 mr-2 ml-0" : ""
                              }`}
                          />
                        </Link>
                      </Button>
            ...
            <Card className="glass bg-white/70 dark:bg-slate-900/70 backdrop-blur-md border border-white/20 dark:border-slate-700/50 rounded-3xl">
              ...
              <CardContent className="space-y-3">
                {popularArticles.map((article) => (
                  <div
                    key={article.id}
                    className={`flex items-center justify-between p-3 glass bg-white/40 dark:bg-slate-800/40 rounded-xl hover:bg-white/60 dark:hover:bg-slate-700/60 transition-all cursor-pointer group ${isRTL ? "flex-row-reverse" : ""
                      }`}
                  >
                    <div className={`flex-1 ${isRTL ? "text-right" : ""}`}>
                      <h4
                        className={`font-medium text-sm group-hover:text-primary transition-colors ${isRTL ? "font-arabic" : ""
                          }`}
                      >
                      ...
                      >
                        ...
                        <span
                          className={`text-xs text-muted-foreground ${isRTL ? "font-arabic" : ""
                            }`}
                        >
            ...
            <Card className="glass bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20 backdrop-blur-md border border-green-200/30 dark:border-green-700/30 rounded-3xl">
              <CardHeader>
                ...
                >
                  ...
                  <CardTitle
                    className={`text-lg text-green-800 dark:text-green-200 ${isRTL ? "font-arabic" : ""
                      }`}
                  >
              ...
              <CardContent className="space-y-4">
                <p
                  className={`text-sm text-green-700 dark:text-green-300 ${isRTL ? "font-arabic text-right" : ""
                    }`}
                >
                  {t("contactSupport.description")}
                </p>
                <Button
                  className={`w-full bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white border-0 rounded-full transition-all duration-200 hover:scale-105 ${isRTL ? "font-arabic" : ""
                    }`}
                  asChild
                >
                  <Link href="/support">
              ...
              <CardContent>
                <Accordion type="single" collapsible className="space-y-2">
                  {Array.from({ length: 3 }).map((_, index) => (
                    ...
                    >
                      <AccordionTrigger
                        className={`text-sm font-medium hover:no-underline ${isRTL ? "font-arabic text-right" : ""
                          }`}
                      >
                        {t(`faq.items.${index}.question`)}
                      </AccordionTrigger>
                      <AccordionContent
                        className={`text-sm text-muted-foreground ${isRTL ? "font-arabic text-right" : ""
                          }`}
                      >
        ...
        <div className="mt-16 text-center">
          <Card className="glass bg-gradient-to-r from-blue-500/10 to-indigo-500/10 backdrop-blur-md border border-blue-200/30 dark:border-blue-700/30 rounded-3xl max-w-2xl mx-auto">
            <CardContent className="p-8">
              <h3
                className={`text-2xl font-bold mb-4 ${isRTL ? "font-arabic" : ""
                  }`}
              >
                {t("bottomCta.title")}
              </h3>
              <p
                className={`text-muted-foreground mb-6 ${isRTL ? "font-arabic" : ""
                  }`}
              >
              ...
              >
                <Button
                  className={`bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 text-white border-0 rounded-full transition-all duration-200 hover:scale-105 ${isRTL ? "font-arabic" : ""
                    }`}
                  asChild
                >
                  <Link href="/support">
                ...
                <Button
                  variant="outline"
                  className={`glass bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm border-white/30 dark:border-slate-700/50 hover:bg-white/80 dark:hover:bg-slate-700/80 rounded-full transition-all duration-200 hover:scale-105 ${isRTL ? "font-arabic" : ""
                    }`}
                  asChild
                >
